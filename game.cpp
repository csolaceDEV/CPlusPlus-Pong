...
simulate_game(Input* input, float dt) {
	clear_screen(0x6b46ff);
	draw_rect(0, 0, arena_half_size_x, arena_half_size_y, 0x4a30b0);
	...
	if (current_gamemode == GM_GAMEPLAY) {
		...
		if (!enemy_is_ai) {
			...
			//if (ball_p_y+2.f > player_1_p) player_1_ddp += 1300;
			//if (ball_p_y -2.f < player_1_p) player_1_ddp -= 1300;
		...
		//player right render
		draw_rect(80, player_1_p, player_half_size_x, player_half_size_y, 0x3af5e3);

		//player left render
		draw_rect(-80, player_2_p, player_half_size_x, player_half_size_y, 0x3af5e3);

		//ball render
		draw_rect(ball_p_x, ball_p_y, ball_half_size, ball_half_size, 0xff0000);
	}
	else {
		...
		draw_text("C + + PONG", -73, 40, 2, 0xff0000);
	}
